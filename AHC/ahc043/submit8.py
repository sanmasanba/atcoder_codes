...
class Solver:
    ...
    def is_connected(self, s: Pos, t: Pos):
        k = (s, t) if s <= t else (t, s)
        res = self.is_connected_cache.get(k, False)
        if res:
            return res
        res = self.field.is_connected(s, t)
        self.is_connected_cache[k] = res
        return res
    ...
    def search_station(self) -> tuple[int, int, int, int, int, int]:
        ...
        def calc_score(unique_people_num: int, cost: int, return_money: int) -> float:
            return (return_money*(1+0.04*unique_people_num))/cost
        
        def calc_param(places1, places2):
            return len([i for i in places1 & places2 if not self.connected[i]])
        ...
        def in_func(person_dist, person_idx, max_score):
            ...
            r0, c0, r1, c1 = -1, -1, -1, -1
            dist = person_dist*-1
            if self.connected[person_idx] or self.is_connected(self.home[person_idx], self.workplace[person_idx]):
                self.connected[person_idx] = 1
                return -1, -1, -1, -1, max_score

            rest_of_turn = self.T - (len(self.actions)+1+dist)
            return_money = self.dist_cache[person_idx]*rest_of_turn
            ...
            if home_around_station and workplace_around_station:
                ...
                for home in home_around_station:
                    for workplace in workplace_around_station:    
                        ...
                        distinct_people = calc_param(self.around_home_workplace[home[0]*self.N+home[1]], self.around_home_workplace[workplace[0]*self.N+workplace[1]])
                        score = calc_score(distinct_people, tmp_cost, return_money)
                        if ((dist < rail_count or 100 < self.income) and
                            tmp_cost < return_money and 
                            not self.is_connected(home, workplace) and
                            max_score < score
                        ):
            ...
            if home_around_station or workplace_around_station:
                ...
                for exist_station in self.station:    
                    ...
                    distinct_people = calc_param(new_around_h_w, self.around_home_workplace[exist_station[0]*self.N+exist_station[1]])
                    score = calc_score(distinct_people, tmp_cost, return_money)
                    if ((dist < rail_count or 100 < self.income) and
                        tmp_cost < return_money and
                        max_score < score
                ...
                for dhr, dhc in SEARCH_ZONE:
                    nhr, nhc = hr+dhr, hc+dhc
                    if not (0 <= nhr < self.N) or not (0 <= nhc < self.N):
                           continue
                    for dwr, dwc in SEARCH_ZONE:
                        nwr, nwc = wr+dwr, wc+dwc
                        dist = distance((nhr, nhc), (nwr, nwc))
                        if not (0 <= nwr < self.N) or not (0 <= nwc < self.N):
                            continue
                        distinct_people = calc_param(self.around_home_workplace[nhr*self.N+nhc], self.around_home_workplace[nwr*self.N+nwc])
    ...
    def __del__(self):
        ...
        print(f"# TOTAL_TIME: {time.time()-S_TIME:02.4}", flush=True)
        print(f"# NEWNEW: {NEWNEW_CNT}, NEWEXI: {NEWEXI_CNT}, EXIEXI: {EXIEXI_CNT}", flush=True)
