...
class Solver:
    ...
    def calc_score(self, unique_people_num: int, cost: int, return_money: int, 
                   pos1: tuple[int, int], pos2: tuple[int, int]) -> float:
        return ((return_money*(0.01+0.01*unique_people_num))/
                (cost*(len(self.field.collect_stations(pos1))+len(self.field.collect_stations(pos2))+0.0001)**0.0001))
    ...
    def single_simulater(self, person_dist, person_idx, max_score):
        ...
        if home_around_station:
            ...
            for exist_station in target_station:    
                ...
                score = self.calc_score(distinct_people, tmp_cost, return_money, new_station, exist_station)
        ...
        k = 4
        for (dhr, dhc) in random.sample(SEARCH_ZONE, k=k):          
            nhr, nhc = hr+dhr, hc+dhc
            if not (0 <= nhr < self.N) or not (0 <= nhc < self.N):
                    continue
            for (dwr, dwc) in random.sample(SEARCH_ZONE, k=k):
                ...
                score = self.calc_score(distinct_people, tmp_cost, return_money, (nhr, nhc), (nwr, nwc))
