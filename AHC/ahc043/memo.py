...
def timer(f):
    def in_f(*args, **kwargs):
        s = time.perf_counter()
        res = f(*args, **kwargs)
        print(f"# {(time.perf_counter()-s)*1000:.3}[ms]")
        return res
    return in_f
...
class Solver:
    ...
    def single_simulater(self, person_idx: int, max_score: int) -> list[tuple[int,int]]:
        ...
        while not (0 <= nhr < self.N) or not (0 <= nhc < self.N):
            ...
            dhr, dhc = random.choice(SEARCH_ZONE)
        ...
        while not (0 <= nwr < self.N) or not (0 <= nwc < self.N):
            ...
            dwr, dwc = random.choice(SEARCH_ZONE)
    ...
    def find_best_rectangular_route(self, r0: int, c0: int, r1: int, c1: int) -> list[tuple[int, int, int]]:
        ...
        # ルートの生成と評価
        typed_vertical, score_v = generate_path(vertical_first)
        typed_horizontal, score_h = generate_path(horizontal_first)
